  ...
  static diffLines(text1, text2) {
    ...
    while (i < lines1.length || j < lines2.length) {
      if (i < lines1.length && j < lines2.length && lines1[i] === lines2[j]) {
        ...
        i++;
        j++;
      } else if (i < lines1.length && (j >= lines2.length || !this.isInLCS(lines1[i], j, lines2, lcs))) {
        // Line removed
...
function runTests() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    try {
      ...
      console.log(`Result: ${stats.additions} additions, ${stats.deletions} deletions, ${stats.unchanged} unchanged`);
