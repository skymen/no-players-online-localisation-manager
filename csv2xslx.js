 ...
 * @param {Object} options - Configuration options (same as csvToXlsx)
 * @returns {Promise<Blob|string>} Returns a Blob if returnBlob is true, otherwise returns blob URL
 *
 * @example
 ...
 *
 * await csvFileToXlsx(file, {
 *   filename: 'converted.xlsx',
 *   title: 'My Data'
 * });
 */
export async function csvFileToXlsx(file, options = {}) {
  ...
  // Use default filename from the file if not provided
  const defaultFilename = file.name.replace(/\.csv$/i, ".xlsx");
  const mergedOptions = {
    filename: defaultFilename,
    ...options,
  };

  return csvToXlsx(csvData, mergedOptions);
}

/**
 * Utility function to detect CSV separator
 *
 * @param {string} csvData - The CSV data
 * @returns {string} Detected separator
 */
export function detectCsvSeparator(csvData) {
